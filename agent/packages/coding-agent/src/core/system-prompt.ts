...
const TASK_DISCOVERY_CACHE_MAX_ENTRIES = 64;
const taskDiscoveryCache = new Map<string, string>();

function buildTaskDiscoveryCacheKey(taskText: string, cwd: string): string {
	return `${cwd}\n${taskText}`;
}

function cacheTaskDiscoverySection(cacheKey: string, value: string): string {
	if (!taskDiscoveryCache.has(cacheKey) && taskDiscoveryCache.size >= TASK_DISCOVERY_CACHE_MAX_ENTRIES) {
		const oldestKey = taskDiscoveryCache.keys().next().value;
		if (oldestKey !== undefined) {
			taskDiscoveryCache.delete(oldestKey);
		}
	}
	taskDiscoveryCache.set(cacheKey, value);
	return value;
}
...
function buildTaskDiscoverySection(taskText: string, cwd: string): string {
	const cacheKey = buildTaskDiscoveryCacheKey(taskText, cwd);
	const cached = taskDiscoveryCache.get(cacheKey);
	if (cached !== undefined) {
		return cached;
	}
	...
	try {
		...
		if (filtered.length === 0 && paths.size === 0) {
			return cacheTaskDiscoverySection(cacheKey, "");
		}
	...
	return cacheTaskDiscoverySection(cacheKey, discoverySection);
}
