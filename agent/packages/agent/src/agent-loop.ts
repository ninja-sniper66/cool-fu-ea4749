...
async function runLoop(
	...
	const nudgeLastSeenAt = new Map<string, number>();
	const queueNudge = (text: string, key: string, cooldownMs = 20_000): boolean => {
		const now = Date.now();
		const last = nudgeLastSeenAt.get(key) ?? 0;
		if (now - last < cooldownMs) return false;
		nudgeLastSeenAt.set(key, now);
		...
		while (hasMoreToolCalls || pendingMessages.length > 0) {
			...
			if (!hasMoreToolCalls && emptyTurnRetries < EMPTY_TURN_MAX) {
				...
				if (tokenCapped || idleStopped) {
					...
					queueNudge(
						...
						tokenCapped ? "empty-turn-token-capped" : "empty-turn-idle",
						15_000,
					);
					continue;
				}
			...
			if (!hasMoreToolCalls && hasProducedEdit && coverageRetries < MAX_COVERAGE_RETRIES) {
				...
				if (missing.length > 0) {
					...
					queueNudge(
						`DO NOT STOP yet. Unedited candidate files: ${list}. Continue only if an explicit acceptance criterion, named path, or required wiring is still unmet. If current edits already satisfy all criteria, reply "done".`,
			...
			if (!hasMoreToolCalls && hasProducedEdit && expectedCriteriaCount > 0 && criteriaCoverageRetries < MAX_CRITERIA_COVERAGE_RETRIES) {
				...
				if (editedFileCount < expectedCriteriaCount) {
					...
					queueNudge(
						`Numeric coverage guardrail: task has ${expectedCriteriaCount} acceptance criteria but only ${editedFileCount} successfully edited file(s) so far (gap: ${gap}). Re-check every criterion now and continue editing if any criterion is still unmet. If all criteria are truly satisfied in fewer files, reply "done".`,
						"criteria-coverage-guardrail",
						20_000,
					);
					continue;
				}
			...
			if (hasMoreToolCalls) {
				...
				for (let i = 0; i < toolResults.length; i++) {
					...
					if (tc.name === "write") {
						...
						if (tr.isError) {
							queueNudge(
								`Write failed for \`${targetPath}\`. Check path and arguments; retry with \`write\` or switch to \`edit\` on an existing file.`,
								`write-fail:${targetPath}`,
								15_000,
							);
							continue;
						}
					...
					if (tr.isError) {
						...
						if (/\d+ occurrences/.test(errText)) {
							queueNudge(
								`Edit failed: oldText matches multiple locations in \`${targetPath}\`. Add more surrounding lines to your oldText to make it unique. Use \`read\` with a small \`limit\`/\`offset\` to see the exact context at the right location.`,
								`edit-fail-occurrences:${targetPath}`,
								15_000,
							);
						} else if (errText.includes("overlap")) {
							queueNudge(
								`Edit failed: the edit ranges in \`${targetPath}\` overlap. Split into separate non-overlapping edits, each targeting a distinct block of text.`,
								`edit-fail-overlap:${targetPath}`,
								15_000,
							);
							...
							queueNudge(
								`Edit schema error on \`${targetPath}\`. Both oldText and newText must be non-empty strings. Format: { "path": "file", "edits": [{ "oldText": "exact non-empty match", "newText": "replacement" }] }. Re-read the file and retry with the correct format.`,
								`edit-fail-schema:${targetPath}`,
								15_000,
							);
						} else if (errText.includes("Could not find") && pendingMessages.length === 0) {
							queueNudge(
								`Edit failed on \`${targetPath}\` — your oldText doesn't match the file. Call \`read\` on \`${targetPath}\` with a small \`limit\` and \`offset\` to see the exact text at the target location, then copy it precisely.`,
								`edit-fail-not-found:${targetPath}`,
								15_000,
							);
						} else if (anchorText && prevAnchor === anchorText && pendingMessages.length === 0) {
							queueNudge(
								`Identical oldText failed twice on \`${targetPath}\`. Use \`read\` to get fresh contents before retrying.`,
								`edit-fail-identical-anchor:${targetPath}`,
								15_000,
							);
						}
					...
					if ((tr.toolName === "find" || tr.toolName === "grep") && tr.isError) {
						...
						if (errText.includes("fd is not available") || errText.includes("ripgrep") || errText.includes("not available")) {
							...
							if (tcFind) {
								...
								queueNudge(
									`The ${tr.toolName} tool is unavailable. Use bash instead:\n\`\`\`\n${bashCmd}\n\`\`\`\nRun this with \`bash\` now.`,
									`tool-unavailable:${tr.toolName}`,
									20_000,
								);
							}
						}
				...
				if (workPhase === "search") {
					for (const tr of toolResults) {
						if (tr.toolName === "bash" && !tr.isError) {
							...
							if (paths.length > 0) {
								...
								workPhase = "absorb";
								queueNudge(
									`Located ${foundFiles.length} candidate files. Read each file you intend to modify before making any edit:\n${[...foundFiles].slice(0, 10).map((p: string) => `- ${p}`).join("\n")}`,
					...
					if (workPhase !== "apply") {
						...
						if (absorbedFiles.size >= absorbLimit && workPhase === "absorb" && pendingMessages.length === 0) {
							workPhase = "apply";
							queueNudge(
								`${absorbedFiles.size} files absorbed. Begin changing the first target file now — invoke \`edit\` (existing files) or \`write\` (new files). Cover acceptance criteria first; open additional files only when a criterion, named path, or required wiring requires it.`,
								"absorb-limit-reached",
								20_000,
							);
						}
					}
